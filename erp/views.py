...
@transaction.atomic
def _sync_marketplace_settlements(channel, rows):
    for row in rows:
        MarketplaceSettlement.objects.update_or_create(channel=channel, external_key=row["external_key"], defaults=row)
    state, _ = MarketplaceOrderSyncState.objects.get_or_create(channel=channel)
    state.last_synced_at = timezone.now(); state.last_error = ""; state.save()
    return len(rows)
...
def marketplace_order_sync(request, channel):
    ...
    if not config["configured"]:
        ...
        try:
            ...
            count = _sync_marketplace_settlements(channel, rows)
            messages.success(request, f"{config['label']} 정산 매출 {count:,}건을 반영했습니다. 기존 내역은 중복 없이 갱신했습니다.")
...
def gold_ledger_list(request):
    ...
    purchases = PurchaseEntry.objects.filter(is_deleted=False).select_related("supplier", "material", "batch")
    if effective_start:
    ...
    for item in purchases:
        memo_parts = [item.supplier.name]
        if item.item_name:
            memo_parts.append(item.item_name)
        if not item.deduct_from_gold_balance:
            memo_parts.append("기존 매입 · 시제 미차감")
        rows.append({
            "date": item.purchase_date, "type": "purchase_issue",
            "type_label": "매입처 금결제" if item.deduct_from_gold_balance else "기존 매입 (시제 미차감)",
            "material": item.material, "actual": item.actual_weight,
            "pure": item.pure_gold_weight,
            "effect": -item.pure_gold_weight if item.deduct_from_gold_balance else Decimal("0"),
            "source": item.reference_no, "memo": " · ".join(memo_parts),
            "image": item.image or (item.batch.image if item.batch_id else None), "manual": None,
        })
    if entry_type:
    ...
    for row in rows:
        if row["type"] == "purchase_issue" and row["effect"] < 0:
            summary["purchase_issue"] += row["pure"]
        elif row["type"] in summary and row["type"] != "purchase_issue":
    ...
    return render(request, "erp/gold_ledger_list.html", {
        ...
        "entry_types": [("customer_payment", "거래처 금 결제"), ("own_factory_issue", "우리공장 금 불출"), ("supplier_issue", "매입처 금 불출"), ("purchase_issue", "매입처 금결제·매입"), ("adjustment", "재고 조정")],
